#!/usr/bin/env python3
"""Evaluate frozen Predictive-CEPA (CEPA-Net v2) on one held-out experiment.

Design and development gate are frozen in
reports/cepa_v1_diagnosis_and_v2_freeze.md before v2 scores are inspected.
"""
...
class CEPANetV2(nn.Module):
    """Predictive CEPA: the aligned bottleneck is the predictor bottleneck."""
    def __init__(self, h: int = 24, bottleneck_dim: int = 32):
        super().__init__()
        ...
        self.bottleneck = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(0.1),
            nn.Linear(2 * h, bottleneck_dim),
            nn.GELU(),
        )
        self.tau = nn.Linear(bottleneck_dim, 1)
        self.risk = nn.Linear(bottleneck_dim, len(HORIZONS_S))

    def forward(self, x: torch.Tensor):
        s = self.state_enc(x)
        d = self.dynamic_enc(dynamic_channels(x))
        z = F.gelu(self.fuse(torch.cat([s, d, s - d, s * d], dim=1)))
        z = self.post(z)
        pooled = torch.cat([z.mean(-1), z.amax(-1)], dim=1)
        hp = self.bottleneck(pooled)
        progress = F.normalize(hp, p=2, dim=1, eps=1e-6)
        tau_n = self.tau(hp).squeeze(-1)
        raw = self.risk(hp)
        mono_logits = torch.cat([
            raw[:, :1],
            raw[:, :1] + torch.cumsum(F.softplus(raw[:, 1:]), dim=1),
        ], dim=1)
        return tau_n, mono_logits, progress
...
def predict(model, x: np.ndarray, device: torch.device):
    ...
    with torch.no_grad():
        for i in range(0, len(x), 128):
            p, r, z = model(torch.from_numpy(x[i:i+128]).to(device))
            ps.append(p.cpu().numpy())
            risks.append(torch.sigmoid(r).cpu().numpy())
            zs.append(z.cpu().numpy())
    return np.concatenate(ps), np.concatenate(risks), np.concatenate(zs)
...
def main():
    ...
    for seed in args.seeds:
        ...
        pn, risk, zte = predict(model, xte, torch.device('cpu'))
        _, _, ztr = predict(model, xtr, torch.device('cpu'))
        ...
        for i in range(len(yte)):
            rec = {'heldout': args.heldout, 'seed': seed, 'tau_true_s': float(yte[i]), 'tau_pred_s': float(pred[i])}
    ...
    audit = {
        ...
        'predictive_alignment': 'TTV and warning heads operate directly on the aligned 32-D bottleneck',
    }
